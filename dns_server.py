...
IP=record_address
D = DomainName('student-project.com.')
#IP = '1.2.3.4'
TTL = 60 * 5

soa_record = SOA(
    mname=(getattr(D, "_acme-challenge")),  # primary name server
    rname=D.simran,  # email of the domain administrator
    times=(
...
ns_records = [NS(getattr(D, "_acme-challenge")), NS(D.ns1), NS(D.ns2)]

records = {
    D: [A(IP), AAAA((0,) * 16), MX(D.mail), soa_record] + ns_records,
    D.ns1: [A(IP)],  # MX and NS records must never point to a CNAME alias (RFC 2181 section 10.3)
    D.ns2: [A(IP)],
#    getattr(D, "_acme-challenge"): [A(IP)],
    D.mail: [A(IP)],
    D.simran: [CNAME(D)],
}
...
def dns_response(data):
    ...
    if qn=="delete":
        records1=[]
    for ans1 in request.__dict__['rr']:
        b=str(ans1).split()[0]
        tok=str(ans1).split()[-1]
        if b==qn:
            records[qn]= tok
            records1.append(ans1)
    if qn=="delete":
        records1=[]
    for rec in records1:
        reply.add_answer(rec)
        reply.add_auth(rec)
